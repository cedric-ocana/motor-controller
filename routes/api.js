...
router.route('/adc-one')
	.get(function(req, res){
		hardware.getAdcValue(null, function getAdcValueCb(err, result){
			generateResponse(err, 'adc-one-get', result, function send(err, data){
			data.data = result;
			res.json(data);
			});
		});
...
router.route('/adc-two')
	.get(function(req, res){
		hardware.getAdc2Value(null, function getAdc2ValueCb(err, result){
			generateResponse(err, 'adc-two-get', result, function send(err, data){
			data.data = result;
			res.json(data);
			});
		});
...
router.route('/power')
        .delete(function (req, res){exec('poweroff',null,function (error, stdout, stderr){});})
        .get(function (req, res){exec('poweroff',null,function (error, stdout, stderr){});});
